...
export class RepositorioTerminalesDB implements RepositorioTerminales {
  ...
  async visualizarRutas(param: any, id: number): Promise<{ rutas: RespuestaRutas[], paginacion: Paginador}> {
    ...
    const consulta = await Database.rawQuery(`SELECT
      ...
      tr.trt_codigo_cp_origen as cp_origen,
      tr.trt_codigo_cp_destino as cp_destino,
      tm.tms_codigo_municipio as municipio_origen,
      tmd.tms_codigo_municipio as municipio_destino,
      td.tdp_codigo_departamento as departamento_origen,
      tdd.tdp_codigo_departamento as departamento_destino,
      ttd.ttd_id as tipo_llegada,
      tn.tnd_id as direccion,
      trev.rev_via as via,
